...
{
    public class BowserFlamethrower : IProjectile
    {
        ...
        public BowserFlamethrower(Vector2 loc, bool facingRight, IPlayer m)
        {
            Location = loc;
            if (facingRight)
                sprite = SpriteFactory.CreateRightFFSprite();
            else
                sprite = SpriteFactory.CreateLeftFFSprite();
            if (facingRight)
            {
