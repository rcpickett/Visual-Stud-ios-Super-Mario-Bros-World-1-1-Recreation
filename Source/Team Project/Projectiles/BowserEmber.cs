...
{
    public class BowserEmber : IProjectile
    {
        ...
        public BowserEmber(Vector2 loc, bool facingRight, IPlayer m)
        {
            Location = loc;
            if(facingRight)
                sprite = SpriteFactory.CreateRightEmberSprite();
            else
                sprite = SpriteFactory.CreateLeftEmberSprite();
            if (facingRight)
            {
                Movement = new Vector2(Speed.Three, Speed.Zero);
            }
            else
            {
                Movement = new Vector2(Speed.NegativeThree, Speed.Zero);
            }
            mario = m;
        }
