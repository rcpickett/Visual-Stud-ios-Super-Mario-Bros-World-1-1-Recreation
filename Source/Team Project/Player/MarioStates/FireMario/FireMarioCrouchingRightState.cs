...
{
    public class FireMarioCrouchingRightState : IPlayerState
    {
        ...
        public FireMarioCrouchingRightState(Mario m)
        {
            sprite = SpriteFactory.CreateFireMarioCrouchingRightSprite();
            mario = m;
        }
        ...
        public void PipeCollision(IPipe pipe, CollisionDetector.CollisionType collision, Vector2 marioLocation, Vector2 camLocation)
        {
            if (IsWarping(marioLocation) && IsInMiddleOfPipe(mario, pipe))
            {
                ...
                {
                    mario.State = new FireMarioGoingDownPipeRightState(mario, marioLocation, camLocation);
                }
            }
