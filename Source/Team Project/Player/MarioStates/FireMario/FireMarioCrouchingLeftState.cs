...
{
    public class FireMarioCrouchingLeftState : IPlayerState
    {
        ...
        public FireMarioCrouchingLeftState(Mario m)
        {
            sprite = SpriteFactory.CreateFireMarioCrouchingLeftSprite();
            mario = m;
        }
        ...
        public void PipeCollision(IPipe pipe, CollisionDetector.CollisionType collision, Vector2 marioLocation, Vector2 camLocation)
        {
            if (IsWarping(marioLocation) && IsInMiddleOfPipe(mario, pipe))
            {
                ...
                {
                    mario.State = new FireMarioGoingDownPipeLeftState(mario, marioLocation, camLocation);
                }
            }
