...
{
    public class BigMarioCrouchingRightState : IPlayerState
    {
        ...
        public BigMarioCrouchingRightState(Mario m)
        {
            mario = m;
            sprite = SpriteFactory.CreateBigMarioCrouchingRightSprite();
        }
        ...
        public void PipeCollision(IPipe pipe, CollisionDetector.CollisionType collision, Vector2 marioLocation, Vector2 camLocation)
        {
            if (IsWarping(marioLocation) && IsInMiddleOfPipe(mario, pipe))
            {
                ...
                {
                    mario.State = new BigMarioGoingDownPipeRightState(mario, marioLocation, camLocation);
                }
            }
